...
import (
	...
	templates "github.com/bookweb/interfacer/internal/engines/gotemplates"
...
type interfaceGenParameters struct {
	Interface    string
	Receiver     string
	Struct       string
	Field        string
...
func Generate(fs afero.Fs, src *ParsedSource, options ...Option) error {
	...
	itf, err := g.generateInterface(src.Structs)
	if err != nil {
		return err
	}
	...
	return g.writer.write(src.Package.Name, imports, accessors, itf)
}
...
func (g *generator) outputFilePath(dir string) string {
	...
	if output == "" {
		...
		output = strings.ToLower(fmt.Sprintf("%s.gen.go", name))
	}
...
func (g *generator) createInterfaceGenParameters(st *Struct, field *Field) *interfaceGenParameters {
	typeName := g.typeName(field.Type)
	getter, setter := g.methodNames(field)
	return &interfaceGenParameters{
		Interface:    fmt.Sprintf("I%s", st.Name),
		Receiver:     g.receiverName(st.Name),
		Struct:       st.Name,
		Field:        field.Name,
		GetterMethod: getter,
...
func (g *generator) generateInterface(structs []*Struct) (string, error) {
	itf := ""
	...
	for _, st := range structs {
		...
		for _, field := range st.Fields {
			...
			if field.Tag.Setter != nil {
				setter, err := g.generateItfSetter(params)
				if err != nil {
					return "", err
				}
				itf += setter
...
func (g *generator) generateItfSetter(
	params *interfaceGenParameters,
) (string, error) {
	t := template.Must(template.New("itf_setter").Parse(templates.ItfSetter))
	buf := new(bytes.Buffer)
...
func (g *generator) generateItfGetter(
	params *interfaceGenParameters,
) (string, error) {
	t := template.Must(template.New("itf_getter").Parse(templates.ItfGetter))
	buf := new(bytes.Buffer)
